...
recursive(inputDir, [], (err, files) => {
  ...
  for(let fileToConvert of filesToConvert) {
    ...
    fs.writeFileSync(fileToConvert, ampify(contents, urlPath, ($) => {
      ...
      let css;
      css = sass.renderSync({file: 'src/styles/common.scss', outputStyle: 'compressed'}).css.toString() + 
            sass.renderSync({file: 'src/styles/typebase.css', outputStyle: 'compressed'}).css.toString();
